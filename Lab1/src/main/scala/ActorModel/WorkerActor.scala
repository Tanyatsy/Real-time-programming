...
import ActorModel.workerProtocol.{RestartException, SendTweetScore, Work, throwException}
    ...
    case Work(temp, id) =>
      ...
      if (temp.contains(": panic")) {
        ...
        if (!tweetEmotions.equals("")) {
          log.info("--------------------------------")
          log.info(s"ActorModel.Tweet emotions word: " + s"${CYAN}" + tweetEmotions + s"${RESET}")
          aggregator ! SendTweetScore(tweetScore, id)
        }
        log.info("--------------------------------")
        log.info(s"ActorModel.Tweet score: " + s"${MAGENTA}" + tweetScore.toString + s"${RESET}")
